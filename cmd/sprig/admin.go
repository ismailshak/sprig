...
func adminInvite(ctx context.Context, getenv func(string) string, stdout io.Writer, handle string) error {
	...
	made, err := auth.IssueSignInLink(ctx, store.New(pool), time.Now(), handle)
	switch {
	...
	case errors.Is(err, auth.ErrNoLiveMembership):
		return fmt.Errorf("%s is not a member of any garden, so invite them from a garden's People page instead", handle)
	case err != nil:
		return err
	}
