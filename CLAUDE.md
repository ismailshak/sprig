...
- A spec file runs in the no-JavaScript project only when the screen it drives has an htmx swap, because there the form-post path is a separate branch of the handler. Those files are listed in `playwright.config.ts`. Within them, `@js` marks a test for behaviour that only exists with JavaScript on and `@nojs` the reverse. Every other screen makes the same request either way and runs once.
