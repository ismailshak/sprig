...
A Playwright test asserts behaviour, and appearance is not behaviour. It drives a browser to find out where a flow lands and what the page says when it arrives. A claim about a stored value belongs in a Go handler test against Postgres, which sees every column and needs no browser. An e2e test never queries the database, and the suite has no way to. Screenshot comparison, pixel diffing and golden images are excluded. A test selects by a role, an accessible name, visible text, or an id the server names as part of a swap contract; never by a CSS class, because renaming one is a design decision and has to stay free. Nothing is mocked, stubbed or intercepted, and assertions are made against the deterministic seed. One test is one flow end to end rather than one page. A card that adds a screen adds the flows that screen makes possible.

A test runs in both Playwright projects, the ordinary one and the one with JavaScript disabled, unless it carries the `@js` tag for behaviour that only exists with JavaScript on. The harness reseeds before every test, so a test asserts against the seed and never against what an earlier test wrote. A test drives a screen through a screen object under `e2e/screens/`, one class per screen, reached as a fixture on the `test` the harness exports. A screen object holds locators and the actions a person performs, never an assertion and never anything that depends on JavaScript. A method on one exists because it encodes something the server names, such as the id format of a care row, or because two tests need it. The seeded people and plants are named in one file under the harness rather than as strings in each test.

A test's name is one claim about the app, stated as a fact the way the Go tests are named: `a signed-out visit is sent to sign in`, `a session survives a reload`. It names a role or a state, never a seeded person, and it never joins two claims with `and`. A name that needs `and` is two tests.
