...
8. **Every flow works as a plain form post and page navigation**, with no JavaScript. With JavaScript, an action that leaves the reader on the page they are on swaps the element that changed and never reloads the page. A whole-page repaint on an in-page action is a bug, whether it comes from a redirect, a GET form or a swap of the body. The swaps are the care row, the undo window, the in-place schedule editor, the lazy photo grid, the Activity filters and pager, Previous and Next on the calendar, the calendar's grid after a note is added, saved or deleted, the Care types section on Garden, the Tokens page, the passkey list, the People page, the Subscribed devices section on Notifications, Save on Account, Garden and People, a changed checkbox or hour on Notifications, Restore on an archived plant's page, and the Remind me later link and its sheet on Today. A new one is checked against this rule first. Back and Forward on Activity and the calendar are handled by htmx and restore the log or the month alone, never the body. Going to another page is a navigation, and `hx-boost` is off. Appearance is the one control that is not a form post. It is kept in localStorage and never posted.
...
10. **A swap replaces only the element that changed, and no intermediate state is painted.** Swapping a larger element than necessary re-renders children that did not change, which replays their entrance animations and loses scroll position, focus and typed input. This is why a refused time on the sheet swaps `#sheet` and not the page behind it. An element swapped under an id it already had needs `settle:0ms`, because otherwise htmx holds the new element's `class` and `style` at the old values for 20ms and the browser paints that frame. Where a change is meant to be seen animating, the animation is on the outgoing element under `htmx-swapping`, with a swap delay long enough to run it.
