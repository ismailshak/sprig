...
10. **A swap replaces only the element that changed, and no intermediate state is painted.** Swapping a larger element than necessary re-renders children that did not change, which replays their entrance animations and loses scroll position, focus and typed input. This is why a Care chip in the sheet swaps `#sheet-form` and not the whole dialog. An element swapped under an id it already had needs `settle:0ms`, because otherwise htmx holds the new element's `class` and `style` at the old values for 20ms and the browser paints that frame. Where a change is meant to be seen animating, the animation is on the outgoing element under `htmx-swapping`, with a swap delay long enough to run it.
