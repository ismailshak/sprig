...
func TestNext(t *testing.T) {
	cases := []struct {
		...
		{
			name:  "a daily anchored series from ten years ago is due the day after the last event",
			sched: anchoredOn(set, 1, UnitDay, time.Date(2016, time.January, 1, 0, 0, 0, 0, time.UTC)),
			last:  event(day(time.September, 2)),
			want:  time.Date(2026, time.September, 3, 0, 0, 0, 0, time.UTC),
		},
		{
			name:  "a three-day anchored series from ten years ago done on an occurrence is due three days later",
			sched: anchoredOn(set, 3, UnitDay, time.Date(2016, time.January, 1, 0, 0, 0, 0, time.UTC)),
			last:  event(day(time.September, 2)),
			want:  time.Date(2026, time.September, 5, 0, 0, 0, 0, time.UTC),
		},
		{
			name:  "a three-day anchored series from ten years ago done a day after an occurrence is due at the next occurrence two days later",
			sched: anchoredOn(set, 3, UnitDay, time.Date(2016, time.January, 1, 0, 0, 0, 0, time.UTC)),
			last:  event(day(time.September, 3)),
			want:  time.Date(2026, time.September, 5, 0, 0, 0, 0, time.UTC),
		},
		{
			name:  "a fortnightly anchored series from ten years ago done on an occurrence is due two weeks later",
			sched: anchoredOn(set, 2, UnitWeek, time.Date(2016, time.January, 4, 0, 0, 0, 0, time.UTC)),
			last:  event(day(time.August, 31)),
			want:  time.Date(2026, time.September, 14, 0, 0, 0, 0, time.UTC),
		},
		{
			name:  "a monthly anchor on the 31st from ten years ago takes February's last day",
			sched: anchoredOn(set, 1, UnitMonth, time.Date(2016, time.January, 31, 0, 0, 0, 0, time.UTC)),
			last:  event(day(time.February, 3)),
			want:  time.Date(2026, time.February, 28, 0, 0, 0, 0, time.UTC),
		},
		{
			name:  "a monthly anchor on the 31st from ten years ago done on February's last day is due on 31 March",
			sched: anchoredOn(set, 1, UnitMonth, time.Date(2016, time.January, 31, 0, 0, 0, 0, time.UTC)),
			last:  event(day(time.February, 28)),
			want:  time.Date(2026, time.March, 31, 0, 0, 0, 0, time.UTC),
		},
		{
			name:  "a two-yearly anchored series done in an off year is due in the next occurrence's year",
			sched: anchoredOn(set, 2, UnitYear, time.Date(2016, time.August, 1, 0, 0, 0, 0, time.UTC)),
			last:  event(time.Date(2025, time.August, 3, 8, 0, 0, 0, time.UTC)),
			want:  time.Date(2026, time.August, 1, 0, 0, 0, 0, time.UTC),
		},
		{
			name:  "a two-yearly anchored series done earlier in an occurrence's month is due that month",
			sched: anchoredOn(set, 2, UnitYear, time.Date(2016, time.August, 1, 0, 0, 0, 0, time.UTC)),
			last:  event(time.Date(2024, time.July, 15, 8, 0, 0, 0, time.UTC)),
			want:  time.Date(2024, time.August, 1, 0, 0, 0, 0, time.UTC),
		},
		{
