...
func TestSecurityHeaders_EveryRouteSetsAllFourHeaders(t *testing.T) {
	...
	stranger := New(logger, testSessions(), testPasskeys(), rejectEveryToken, queries, testTemplates(), testAssets(), "", false, testPushKey)
	owner := New(logger, testSessions(), testPasskeys(), acceptEveryToken(memberWith(everyCapability())), queries, testTemplates(), testAssets(), "", false, testPushKey)
