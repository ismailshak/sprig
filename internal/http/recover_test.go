...
func recoverMux(t *testing.T, f *recoverFixture) http.Handler {
	t.Helper()
	return New(testLogger, testSessions(), f.handler.passkeys, rejectEveryToken, noLiveToken, f.queries, testPhotos(t), testTemplates(), testAssets(), "", false, testPushKey, nil, nil)
}
