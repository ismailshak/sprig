...
type access struct {
	...
	// header and not the session cookie. Every other route refuses a token.
	bearer bool
...
func TestRoutes_EveryRouteHasOneRouteAccessEntryThatMatchesIt(t *testing.T) {
	table := routes(testLogger, testSessions(), testPasskeys(), nil, testPhotos(t), testTemplates(), testAssets(), "", false, testPushKey, nil, nil, nil, nil)
	...
	for _, r := range table {
		...
		if a.bearer != r.bearer {
			t.Errorf("%s takes an API token = %v in routeAccess and %v in routes", r.pattern, a.bearer, r.bearer)
		}
		if a.bearer && (a.public || a.capability != "" || a.withoutGarden) {
			t.Errorf("%s takes an API token and is public, requires %q or is served without a garden, and a token is a garden's own read-only credential", r.pattern, a.capability)
		}
		if a.bearer && mutates(method) {
			t.Errorf("%s takes an API token and mutates, and a token can only read", r.pattern)
		}
...
func TestRoutes_EachRouteRefusesStrangersAndRolesAsItsEntrySays(t *testing.T) {
	...
	for _, r := range routes(testLogger, testSessions(), testPasskeys(), nil, testPhotos(t), testTemplates(), testAssets(), "", false, testPushKey, nil, nil, nil, nil) {
		...
		t.Run(r.pattern, func(t *testing.T) {
			...
			}), noLiveToken, queries, testPhotos(t), testTemplates(), testAssets(), "", false, testPushKey, nil, nil, nil, nil)
			...
			case a.bearer && rec.Code != http.StatusUnauthorized:
				t.Errorf("a stranger got %d from a route that takes an API token, want %d", rec.Code, http.StatusUnauthorized)
			case !a.public && !a.bearer && !sentToSignIn:
				t.Errorf("a stranger got %d from %q, want %d to %s", rec.Code, rec.Header().Get("Location"), http.StatusSeeOther, signInPath)
			}

			// A live token is accepted only on the routes flagged bearer.
			// Every other route leaves it unread and sends the device to sign
			// in.
			tokensResolved := 0
			...
			rec = httptest.NewRecorder()
			New(logger, testSessions(), testPasskeys(), rejectEveryToken, tokens, queries, testPhotos(t), testTemplates(), testAssets(), "", false, testPushKey, nil, nil, nil, nil).ServeHTTP(rec, withToken(httptest.NewRequestWithContext(t.Context(), method, path, nil)))
			sentToSignIn = rec.Code == http.StatusSeeOther && rec.Header().Get("Location") == signInPath
			switch {
			...
			case !a.bearer && !a.public && !sentToSignIn:
				t.Errorf("a device's token got %d from %q, want %d to %s", rec.Code, rec.Header().Get("Location"), http.StatusSeeOther, signInPath)
			}

			if a.bearer {
				rec = httptest.NewRecorder()
				New(logger, testSessions(), testPasskeys(), acceptEveryToken(memberWith(every)), noLiveToken, queries, testPhotos(t), testTemplates(), testAssets(), "", false, testPushKey, nil, nil, nil, nil).ServeHTTP(rec, signedIn(httptest.NewRequestWithContext(t.Context(), method, path, nil)))
			...
			if a.capability != "" {
				...
				rec = httptest.NewRecorder()
				New(logger, testSessions(), testPasskeys(), acceptEveryToken(lacking), noLiveToken, queries, testPhotos(t), testTemplates(), testAssets(), "", false, testPushKey, nil, nil, nil, nil).ServeHTTP(rec, signedIn(httptest.NewRequestWithContext(t.Context(), method, path, nil)))
			...
			if !a.public && !a.bearer {
				rec = httptest.NewRecorder()
				New(logger, testSessions(), testPasskeys(), acceptEveryToken(noGardenPrincipal()), noLiveToken, queries, testPhotos(t), testTemplates(), testAssets(), "", false, testPushKey, nil, nil, nil, nil).ServeHTTP(rec, signedIn(httptest.NewRequestWithContext(t.Context(), method, path, nil)))
			...
			if a.foreign != "" {
				rec = httptest.NewRecorder()
				New(logger, testSessions(), testPasskeys(), acceptEveryToken(memberWith(every)), noLiveToken, queries, testPhotos(t), testTemplates(), testAssets(), "", false, testPushKey, nil, nil, nil, nil).ServeHTTP(rec, signedIn(httptest.NewRequestWithContext(t.Context(), method, a.foreign, nil)))
				if rec.Code != http.StatusNotFound {
...
func everyCapability() auth.Capabilities {
	...
	for _, r := range routes(testLogger, testSessions(), testPasskeys(), nil, nil, testTemplates(), testAssets(), "", false, testPushKey, nil, nil, nil, nil) {
		if r.capability != "" {
