...
func TestRoutes_EveryRouteHasOneRouteAccessEntryThatMatchesIt(t *testing.T) {
	table := routes(testDependencies(t))
...
func TestRoutes_EachRouteRefusesStrangersAndRolesAsItsEntrySays(t *testing.T) {
	...
	for _, r := range routes(testDependencies(t)) {
		...
		t.Run(r.pattern, func(t *testing.T) {
			// app returns New's handler on the route test database.
			app := func(resolver, tokens Resolver) http.Handler {
			...
			app(ResolverFunc(func(context.Context, time.Time, string) (auth.Principal, error) {
				resolved++
				return auth.Principal{}, auth.ErrNoSession
			}), noLiveToken).ServeHTTP(rec, httptest.NewRequestWithContext(t.Context(), method, path, nil))
			...
			rec = httptest.NewRecorder()
			app(rejectEveryToken, tokens).ServeHTTP(rec, withToken(httptest.NewRequestWithContext(t.Context(), method, path, nil)))
			sentToSignIn = rec.Code == http.StatusSeeOther && rec.Header().Get("Location") == signInPath
			switch {
			...
			if a.bearer {
				rec = httptest.NewRecorder()
				app(acceptEveryToken(memberWith(every)), noLiveToken).ServeHTTP(rec, signedIn(httptest.NewRequestWithContext(t.Context(), method, path, nil)))
			...
			if a.capability != "" {
				...
				rec = httptest.NewRecorder()
				app(acceptEveryToken(lacking), noLiveToken).ServeHTTP(rec, signedIn(httptest.NewRequestWithContext(t.Context(), method, path, nil)))
			...
			if !a.public && !a.bearer {
				rec = httptest.NewRecorder()
				app(acceptEveryToken(noGardenPrincipal()), noLiveToken).ServeHTTP(rec, signedIn(httptest.NewRequestWithContext(t.Context(), method, path, nil)))
			...
			if a.foreign != "" {
				rec = httptest.NewRecorder()
				app(acceptEveryToken(memberWith(every)), noLiveToken).ServeHTTP(rec, signedIn(httptest.NewRequestWithContext(t.Context(), method, a.foreign, nil)))
				if rec.Code != http.StatusNotFound {
...
func everyCapability() auth.Capabilities {
	...
	for _, r := range routes(Dependencies{Logger: testLogger, Sessions: testSessions(), Passkeys: testPasskeys(), Templates: testTemplates(), Assets: testAssets()}) {
		if r.capability != "" {
