...
func TestNew_HealthzOK(t *testing.T) {
	...
	handler := New(logger, testSessions(), testPasskeys(), rejectEveryToken, nil, testTemplates(), testAssets(), "", false)
...
func TestNew_UnknownRouteIsSignInForAStrangerAndNotFoundForAMember(t *testing.T) {
	logger := slog.New(slog.NewJSONHandler(io.Discard, nil))
	handler := New(logger, testSessions(), testPasskeys(), acceptEveryToken(sitterPrincipal()), nil, testTemplates(), testAssets(), "", false)
