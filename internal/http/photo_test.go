...
func photosServedTo(t *testing.T, queries *store.Queries) servedPhotos {
	...
	f.handler = New(testLogger, testSessions(), testPasskeys(), acceptEveryToken(memberWith(everyCapability())), noLiveToken, queries, photos, testTemplates(), testAssets(), "", false, testPushKey, nil, nil)
	return f
}
